...
const handleLaneCreation = async (pickup, delivery, customer_id) => {
  ...
  if (lane === null || lane === undefined || lane.length === 0) {
    // let distance = await Geocode.Geocode.getCoordinatesAndDistance(pickup.address, delivery.address);
    let newLane = {
      customer_id: customer_id,
      description: `${pickup.address} to ${delivery.address}`, //name of first location + to + name of second location
  ...
  if (reverseLane === null || reverseLane === undefined || reverseLane.length === 0) {
    newReverseLane = {
      customer_id: customer_id,
      description: `${delivery.address} to ${pickup.address}`, //name of first location + to + name of second location
