...
const createTookanLocation = (move) => {
  axios({
    method: 'post',
    url: 'https://test-api.exampleurl.com/v2/customer/add',
    data: {
      "api_key": "example",
      "user_type": 0,
...
const getTookanLocation = async(id) => {
  axios({
    method: 'post',
    url: 'https://test-api.exampleurl/v2/view_customer_profile',
    data: {
      "api_key": "example",
      "customer_id": id
    },
...
const createMove = (move, callback) => {

  const deliverytime = new Date(move.delivery_time).toISOString();
  const pickuptime = new Date(move.pickup_time).toISOString();

  axios({
    method: 'post',
    url: 'https://test-api.exampleurl.com/v2/create_multiple_tasks',
    data: {
      api_key: 'example',
      // fleet_id: 1,
      timezone: +300,
